...
class SmartTurnVADService:
    def __init__(self, model_name="pipecat-ai/smart-turn-v2", device="cuda", buffer_duration=8.0):
        ...
        self.audio_buffer = deque(maxlen=int(self.buffer_duration * self.sample_rate))
        self.buffer_lock = threading.Lock()
        
        # VAD pipeline
        self.vad_pipeline: Optional[pipeline] = None
        self.last_turn_check = 0
        self.turn_check_interval = 0.25  # Check every 250ms for responsiveness
        self.confidence_threshold = 0.4  # Much lower threshold for better detection
        ...
        self.max_speech_duration = 15.0  # Force completion after 15 seconds
        self.silence_timeout = 3.0  # Complete after 3 seconds of silence
    ...
    def add_pcm_chunk(self, pcm_data: np.ndarray) -> None:
        ...
        logger.debug(f"Added {len(audio_float)} samples, buffer size: {len(self.audio_buffer)} ({len(self.audio_buffer) / self.sample_rate:.1f}s), speech={has_speech}")

    def get_buffered_audio(self) -> np.ndarray:
        """Get current audio buffer as numpy array (thread-safe)"""
        with self.buffer_lock:
    ...
    def is_turn_complete(self) -> Tuple[bool, float, str]:
        ...
        # Fallback: Force completion after max speech duration
